...
import csv
csv_reader = csv.reader(open('data_06.csv', encoding='utf-8'))
...
for row in csv_reader:
    if(row[2] == '我' and row[5] == '文本'):
        print(row[2],row[6])
        signature_pre = row[6].strip().replace("@我在建湖想小心", "").replace("@我爱周星驰", "").replace("@小心我来了", "").replace("@我要去西藏", "")
